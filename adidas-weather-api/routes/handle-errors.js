const { LogicError, ValueError, ParameterError, UnknownError, UnauthorizedError } = require('../common/errors')
...
async function handleErrors(callback, res) {
    try {
        await callback()
            ...
            if (error instanceof TypeError || error instanceof ValueError || error instanceof ParameterError) status = 406
